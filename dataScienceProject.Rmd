...
bostonmap <- get_stamenmap(bbox, zoom = 12, maptype = "toner-lite")  # terrain fails
ggmap(bostonmap)
ggmap(bostonmap) +
...
```{r}
qmplot(latitude, longitude, data = seattle_table, 
       colour = I('red'), size = I(1), darken = .1)
seattlebox <-  c(left = min(seattle_table$latitude), bottom = min(seattle_table$latitude), 
          right = max(seattle_table$Long), top = max(seattle_table$Lat))
seattlemap <- get_stamenmap(seattlebox, zoom = 12, maptype = "toner-lite")
ggmap(seattlemap)
