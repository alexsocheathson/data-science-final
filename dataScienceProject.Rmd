...
```{r}
count_price <- ggplot(data = San_Francisco_table) +
...
```{r}
count_rating <- ggplot(data = San_Francisco_table) +
...
```{r}
count_review <- ggplot(data = San_Francisco_table) +
...
```{r}
rating_review <- ggplot(data = San_Francisco_table) +
...
## Not sure where $$$$ went 
```{r}
rating_price <- ggplot(data = San_Francisco_table) +
...
```{r}
price_review <- ggplot(data = San_Francisco_table) +
...
seattle_box <- c(left = min(seattle_table$longitude), bottom =
min(seattle_table$latitude), right = max(seattle_table$longitude), top = max(seattle_table$latitude))

seattlemap <- get_stamenmap(seattle_box, zoom = 12, maptype = "terrain") 
ggmap(seattlemap)

ggmap(seattlemap) +
  geom_point(data = seattle_table, aes(x = longitude, y = latitude))
```
...
houston_box <- c(left = min(Houston_table$longitude), bottom =
min(Houston_table$latitude), right = max(Houston_table$longitude), top = max(Houston_table$latitude))

houstonmap <- get_stamenmap(houston_box, zoom = 12, maptype = "terrain") 
ggmap(houstonmap)

ggmap(houstonmap) +
  geom_point(data = Houston_table, aes(x = longitude, y = latitude))
```
...
nyc_box <- c(left = min(new_york_table$longitude), bottom =
min(new_york_table$latitude), right = max(new_york_table$longitude), top = max(new_york_table$latitude))

nycmap <- get_stamenmap(nyc_box, zoom = 12, maptype = "terrain") 
ggmap(nycmap)

ggmap(nycmap) +
  geom_point(data = new_york_table, aes(x = longitude, y = latitude))
```
